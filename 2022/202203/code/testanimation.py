...
class test_last_bezier_4minus4(ThreeDScene):
    def setup(self):
        axes = ThreeDAxes(x_range=np.array([-5.0, 5.0, 1.0]),
                          y_range=np.array([-5.0, 5.0, 1.0]),
                          z_range=np.array([-4.0, 4.0, 1.0])
        ...
        B_3 = Dot3d(color=RED).add_updater(
            lambda m: m.move_to((R13.get_center() - R03.get_center()) * t.get_value() + R03.get_center()))
        B_group = VGroup(B,B_1,B_2,B_3).set_color(WHITE)
        self.add(B_group)
        P0_P1E = Line().add_updater(lambda m : m.put_start_and_end_on(B_1.get_center() , B.get_center()))
        P1_P2E = Line().add_updater(lambda m : m.put_start_and_end_on(B_2.get_center() , B_1.get_center()))
        P2_P3E = Line().add_updater(lambda m : m.put_start_and_end_on(B_3.get_center() , B_2.get_center()))
        P_Elines = VGroup(P0_P1E, P1_P2E, P2_P3E).set_color(WHITE)

        Q0E = Dot3d(color=BLUE).add_updater(lambda m: m.move_to((B_1.get_center() - B.get_center()) * t.get_value() + B.get_center()))
        Q1E = Dot3d(color=BLUE).add_updater(lambda m: m.move_to((B_2.get_center() - B_1.get_center()) * t.get_value() + B_1.get_center()))
        Q2E = Dot3d(color=BLUE).add_updater(lambda m: m.move_to((B_3.get_center() - B_2.get_center()) * t.get_value() + B_2.get_center()))
        QE = VGroup(Q0E, Q1E, Q2E).set_color(Google_GREEN)
        ...
        R0E = Dot3d(color=GREEN).add_updater(lambda m: m.move_to((Q1E.get_center() - Q0E.get_center()) * t.get_value() + Q0E.get_center()))
        R1E = Dot3d(color=GREEN).add_updater(lambda m: m.move_to((Q2E.get_center() - Q1E.get_center()) * t.get_value() + Q1E.get_center()))
        RE = VGroup(R0E, R1E).set_color(Google_BLUE)
        ...
        pathE = TracedPath(BE.get_center, stroke_width=7, stroke_color=GOLD)
        #todo 我这里没有实现理想的原因是因为我这里的贝塞尔曲线是模拟整个过程中的运动，整个过程只花了一条贝塞尔曲线;
        # 而mathbox那个是对每时每刻的都画了一条，所以我的没毛病，只是部分需要改进,之后再做吧
        path = TracedPath(B.get_center, stroke_width=1, stroke_color=RED)
        path_1 = TracedPath(B_1.get_center, stroke_width=1, stroke_color=RED)
        path_2 = TracedPath(B_2.get_center, stroke_width=1, stroke_color=RED)
        path_3 = TracedPath(B_3.get_center, stroke_width=1, stroke_color=RED)
