...
class A(Scene):
    def construct(self):
        ...
        self.wait()
        self.play(
            text_group.arrange, UP, {"aligned_edge": RIGHT, "buff": 2}
        )
        self.wait()
        self.play(
            text_group.arrange, RIGHT, {"aligned_edge": DOWN, "buff": 2}
        )
        self.wait()
        self.play(
            text_group.arrange, LEFT, {"aligned_edge": UP, "buff": 2}
        )
        self.wait()
...
class M (GraphScene, MovingCameraScene):
    CONFIG = {
        "y_max": 50,
        "y_min": 0,
        "x_max": 7,
    ...
    def construct(self):
        ...
        dot_at_middle = Dot().move_to(graph.points[199])
        dot_at_end_grap = Dot().move_to(graph.points[-1])
        print(len(graph.points))

        self.add(graph, dot_at_end_grap, dot_at_middle,dot_at_start_graph)
        ...
        self.play(
            self.camera_frame.move_to, dot_at_middle
        )
        self.play(
