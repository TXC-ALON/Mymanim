...
class Surface_test_01(SpecialThreeDScene):
  ...
  def construct(self):
    self.set_camera_to_default_position()
    axes = self.get_axes(z_min=-5, z_max=5)
...
class Surface_test_02(SpecialThreeDScene):
  ...
  def construct(self):
    self.set_camera_to_default_position()
    axes = ThreeDAxes(z_min=-1, z_max=10)
    w = 1
    ...
    surface_02 = ParametricSurface(lambda u, v: v * complex_to_R3(np.exp(1j * w * u)) + OUT * u / PI * 2,
                                   u_min=0, u_max=TAU, v_min=1, v_max=3, checkerboard_colors=None,
                                   fill_color=GOLD_D, fill_opacity=0.8, stroke_color=YELLOW_C, resolution=(60, 10))
    self.add(axes, surface_01)
    self.wait()
    self.play(TransformFromCopy(surface_01, surface_02, rate_func=linear), run_time=4)
    self.wait(2)
...
class Set_surface_color_test(SpecialThreeDScene):
  ...
  def construct(self):
    ...
    # surface_frame为线框图
    surface_frame = surface_origin.copy().set_fill(color=BLUE, opacity=0.1)
...
class Mobius_to_Heartshape(SpecialThreeDScene):
  ...
  def construct(self):
    self.set_camera_to_default_position()
    heart_curve_func = lambda t: (16 * np.sin(t) ** 3 * RIGHT + (
      13 * np.cos(t) - 5 * np.cos(2 * t) - 3 * np.cos(3 * t) - np.cos(4 * t)) * UP + np.sin(t) * (
                                    1 - abs(-t / PI)) ** 2 * 8 * OUT) * 0.21

    r = 0.5
    heart_shape_mobius = ParametricSurface(
      lambda u, v: heart_curve_func(u) + v * np.cos(u / 2) * np.array([np.cos(u), np.sin(u), 0])
                   + v * np.sin(-u / 2) * OUT,
...
class Wave_of_boxes_02(SpecialThreeDScene):
  ...
  def construct(self):
    ...
    a = 0.1
    amp = 0.9  # amplitude
    self.wave_func = lambda u, v: np.array([u, v,
                                            amp + 0.0001 + amp * np.sin((u ** 2 + v ** 2) / 2 + self.var_phi) * np.exp(
                                              -a * (np.sqrt(u ** 2 + v ** 2)))])
      ...
      for j in range(n):
        ...
        box_ij = Box_02(pos=xyz, box_height=self.wave_func(xyz[0], xyz[1])[-1], color=BLUE,
                        ...
                        fill_color=self.colors[
                          int(np.sqrt(sum(xyz ** 2)) / np.sqrt(x_range ** 2 + y_range ** 2) * 100)])
        boxes.add(box_ij)
    return boxes
