...
class Logo(VGroup):
    ...
    CONFIG = {
        ...
        'color_2': [WHITE, '#C59978', '#8D5630'],'''两种棕色'''
...
class MyTransform(Animation):
    ...
    def clean_up_from_scene(self, scene: object) -> object:
        if self.is_remover():
...
class Logo_Rotate_Out7(Scene):
    ...
    def construct(self):
        ...
        text[1][6].align_to(text[1][8], UP)#起到对齐作用，不然g是下半部分对齐#
        text[1][0].set_color(logo.color_2[2])
        text[0][0].set_color(logo.color_1[2])
        all_logo = VGroup(logo, text).center()
        ...
        #     s.increment_value(s_speed)
        #     self.wait(1/self.camera.frame_rate)
        self.play(s.set_value, 10, rate_func=rush_into, run_time=6)
