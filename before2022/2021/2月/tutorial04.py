...
self.play(UpdateFromAlphaFunc(gup, anim),
    run_time=4)""", t2s={"def": ITALIC}).scale(1.5).to_corner(UL)
...
class VTExample(DarkScene):
    def construct(self):
        ...
        texta = TexMobject(r"add\_updater", plot_depth=2).scale(1.5).shift(UP * 2.5).add_background_rectangle(opacity=0.5)
...
"""
        ...
        codeplay = CodeLine("self.play(...)", t2s={"def": ITALIC, "self": ITALIC}).next_to(code, DOWN, aligned_edge=LEFT)
        codeplay.scale(2).set_color_by_t2c(t2c)
        codewait = CodeLine("self.wait(7)", t2s={"def": ITALIC, "self": ITALIC}).next_to(codeplay, DOWN, aligned_edge=LEFT)
        codewait.scale(2).set_color_by_t2c(t2c)
        self.play(
    ...
    def construct(self):
        ...
        def anim(obj):
            for i in range(num):
                ...
                if distance == 0:
                    obj[i].become(Line(4.5 * np.array([np.cos(i * TAU / num), np.sin(i * TAU / num), 0]),
                                       4.5 *
                                       np.array(
