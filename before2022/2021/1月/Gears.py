...
class Gear_outline(VMobject):
    ...
    def __init__(self, pitch_circle_radius=2, tooth_hight=0.5, tooth_num=17, **kwargs):
        ...
        theta_pitch = np.tan(np.arccos(self.rb / self.pitch_circle_radius)) - np.arccos(
            self.rb / self.pitch_circle_radius)
        theta_top = np.tan(np.arccos(self.rb / (self.rb + self.tooth_hight))) - np.arccos(
            self.rb / (self.rb + self.tooth_hight))
        ...
        arc_top_thetas = np.linspace(theta_top - theta_pitch - TAU / self.tooth_num / 2 / 2,
                                     -(theta_top - theta_pitch - TAU / self.tooth_num / 2 / 2), self.arc_segments)
        curve02_radius = self.rb / np.cos(alphas)
        curve02_thetas = -curve01_thetas
        arc_bottom_thetas = np.linspace(theta_pitch + TAU / self.tooth_num / 2 / 2,
                                        -(theta_pitch + TAU / self.tooth_num / 2 / 2) + TAU / self.tooth_num,
                                        self.arc_segments)
        ...
        part_01_thetas = np.concatenate(
            (curve01_thetas, arc_top_thetas[1:-1], curve02_thetas[::-1], arc_bottom_thetas[1:-1]), axis=0)
        part_01_radius = np.concatenate(
            (curve01_radius, arc_top_radius[1:-1], curve02_radius[::-1], arc_bottom_radius[1:-1]), axis=0)
        ...
        if type(theta) == np.ndarray:
            if type(r) == np.ndarray:
                ...
                return np.concatenate(
                    (np.cos(theta).reshape(-1, 1), np.sin(theta).reshape(-1, 1), theta.reshape(-1, 1) * 0),
                    axis=1) * r.reshape(-1, 1)
            else:
                return np.concatenate(
                    (np.cos(theta).reshape(-1, 1), np.sin(theta).reshape(-1, 1), theta.reshape(-1, 1) * 0), axis=1) * r
        else:
...
class Show_gear(Scene):
    ...
    def construct(self):
        gear = Gear_outline(pitch_circle_radius=3.6, tooth_hight=0.25, tooth_num=67, color=BLUE,
                            stroke_width=2.5)  # .round_corners()

        gear_02 = Gear_outline(pitch_circle_radius=3.6 * 49 / 67, tooth_hight=0.25, tooth_num=49, color=YELLOW,
                               stroke_width=2.5).shift(RIGHT * (3.6 - 3.6 * 49 / 67))
        ...
        def gear_updater(gear, dt):
            ...
            # 点积，np.dot([1,2,3],[4,5,6]) = 1*4 + 2*5 + 3*6  = 32。
...
class Spirograph(VGroup):
    ...
    def __init__(self, **kwargs):
        ...
        self.outer_circle = Circle(radius=self.radius_outer, fill_color=self.gear_color[0], fill_opacity=0.54,
                                   stroke_color=self.gear_color[0], stroke_width=2.25)
        self.outline_big = Gear_outline(pitch_circle_radius=self.radius_big, tooth_num=self.tooth_num_big,
                                        tooth_hight=self.tooth_hight,
                                        stroke_width=2.25, fill_opacity=1, fill_color=BLACK,
                                        stroke_color=self.gear_color[0])
        ...
        self.outline_small = Gear_outline(pitch_circle_radius=self.radius_small, tooth_num=self.tooth_num_small,
                                          tooth_hight=self.tooth_hight,
                                          stroke_width=2.25, stroke_color=self.gear_color[1],
                                          fill_color=self.gear_color[1],
                                          fill_opacity=0.32).move_to(self.center_loc).shift(
            RIGHT * (self.radius_big - self.radius_small - 0.01))
    ...
    def add_hole(self, loc, scale=1):
        ...
        hole = Circle(radius=self.hole_radius, color=self.gear_color[1], fill_color=BLACK, fill_opacity=1,
                      stroke_width=1.8).scale(scale).move_to(self.outline_small.get_center() + loc)
...
class Spirograph_test(Scene):
    ...
    def update_small_gear(self, spirograph, dt=1 / 60):
        ...
        if len(self.temp_dot) == 2:
            spirograph.curves_group.add(Line(self.temp_dot[-1], self.temp_dot[-2], stroke_width=2.4,
                                             color=self.color_list[
                                                 int(self.color_pointer / 20) % len(self.color_list)]))
...
class SpirographScene(Scene):
    ...
    def construct(self):
        ...
        self.spirograph = Spirograph(radius_outer=self.total_hight / 2, radius_big=self.total_hight / 2 - 0.32,
                                     center_loc=self.spirograph_loc,
                                     tooth_num_big=self.tooth_num_big, tooth_num_small=self.tooth_num_small,
    ...
    def draw_last_hole(self, spirograph, dt=1 / 60):
        ...
        if len(self.temp_dot) > 2:
            self.temp_dot.remove(self.temp_dot[0])
        if len(self.temp_dot) == 2:
            spirograph.curves_group.add(Line(self.temp_dot[-1], self.temp_dot[-2], stroke_width=self.curve_stroke,
                                             color=self.color_list[
                                                 int(self.color_pointer / 20) % len(self.color_list)]))
        # self.add(Dot(spirograph.hole_group[-1].get_center()).scale(0.4).set_color(self.color_list[int(self.color_pointer/8)%len(self.color_list)]))
        self.color_pointer += 1
    ...
    def draw_all_hole(self, spirograph, dt=1 / 60):
        ...
        for i in range(n):
            ...
            if len(self.temp_dot[i]) == 2:
                spirograph.curves_group.add(
                    Line(self.temp_dot[i][-1], self.temp_dot[i][-2], stroke_width=self.curve_stroke,
                         color=self.color_list[int(i / n * len(self.color_list))]))
    ...
    def add_parameter_info(self):
        ...
        text_small_num = Text('动轮齿数：', font='新蒂小丸子体', color=self.gear_color[1]).scale(0.64).to_corner(
            UP * 4.5 + LEFT * 18)
        text_ratio = Text('齿数比：', font='新蒂小丸子体', color=average_color(*self.gear_color)).scale(0.64).to_corner(
            UP * 7 + LEFT * 18)
        big_num = Text('%d' % self.tooth_num_big, font='Comic Sans MS', color=self.gear_color[0]).scale(0.64).next_to(
            text_big_num, RIGHT * 1.5)
        small_num = Text('%d' % self.tooth_num_small, font='Comic Sans MS', color=self.gear_color[1]).scale(
            0.64).next_to(text_small_num, RIGHT * 1.5).align_to(big_num, LEFT)
        ratio = Text(self.ratio_str, font='Comic Sans MS', color=average_color(*self.gear_color)).scale(0.64).next_to(
            text_ratio, RIGHT * 1.5)
...
class Spirograph_72_54(SpirographScene):
    CONFIG = {
        'tooth_num_big': 72,
        'tooth_num_small': 54,
        'ratio_str': '4/3',
    ...
    def add_holes(self):
        self.spirograph.hole_radius = 0.072
        self.spirograph.add_hole_by_line(hole_num=14, delta_r=0.275)
...
class Spirograph_72_36(SpirographScene):
    CONFIG = {
        'tooth_num_big': 72,
        'tooth_num_small': 36,
        'ratio_str': '2/1',
    ...
    def add_holes(self):
        self.spirograph.hole_radius = 0.072
        self.spirograph.add_hole_by_line(hole_num=8, delta_r=0.275)
...
class Spirograph_72_24(SpirographScene):
    CONFIG = {
        'tooth_num_big': 72,
        'tooth_num_small': 24,
        'ratio_str': '3/1',
...
class Spirograph_72_18(SpirographScene):
    CONFIG = {
        'tooth_num_big': 72,
        'tooth_num_small': 18,
        'ratio_str': '4/1',
    ...
    def add_holes(self):
        self.spirograph.hole_radius = 0.072
        self.spirograph.add_hole_by_line(hole_num=6, delta_r=0.275)
...
class Spirograph_75_45(SpirographScene):
    CONFIG = {
        'tooth_num_big': 75,
        'tooth_num_small': 45,
        'ratio_str': '5/3',
...
class Spirograph_72_60(SpirographScene):
    CONFIG = {
        'tooth_num_big': 72,
        'tooth_num_small': 60,
        'ratio_str': '6/5',
...
class Spirograph_explain(SpirographScene):
    ...
    def construct(self):
        ...
        self.spirograph.add_hole(RIGHT * 1.6)

        self.pitch_big = Circle(radius=self.spirograph.radius_big, color=BLUE, stroke_width=3, fill_opacity=0).move_to(
            self.spirograph.outline_big.get_center())
        self.pitch_small = Circle(radius=self.spirograph.radius_small, color=YELLOW, stroke_width=3,
                                  fill_opacity=0).move_to(self.spirograph.outline_small.get_center())
        self.center_trace = Circle(radius=self.spirograph.radius_big - self.spirograph.radius_small, color=GREEN,
                                   stroke_width=2, fill_opacity=0).move_to(self.spirograph.outline_big.get_center())
        center = Dot(color=GREEN).scale(1.2).move_to(self.pitch_small.get_center())
        arrow = VGroup(
            Arrow(self.spirograph.outline_small.get_center(), self.spirograph.hole_group[-1].get_center(), buff=0,
                  color=YELLOW))
        ...
        self.play(ApplyMethod(self.spirograph.outline_small.set_opacity, 0.2),
                  # ApplyMethod(self.spirograph.outline_small.set_stroke, opacity=0.2),
...
class Test(Scene):
    ...
    def construct(self):
        o = ORIGIN
        p = RIGHT * 2
        dot = Dot(p, color=GREEN)
        po = Line(p, o, color=RED)

        self.play(ShowCreation(dot))
        dot.add_updater(lambda d: d.move_to(p))
        po.add_updater(lambda l: l.put_start_and_end_on(o, p))
        d_theta = TAU / 360 * 3
        dt = 1 / 30
        self.add(po)
        for i in range(120):
            p = 2 * RIGHT * np.cos(d_theta * i) + 2 * UP * np.sin(d_theta * i)
            self.wait(dt)
        self.wait()
...
class Draw_Ellipse(Scene):
    ...
    def construct(self):
        ...
        dot_F1 = Dot(F1, color=RED).scale(2)
        dot_F2 = Dot(F2, color=RED).scale(2)
        dot_P = Dot(P, color=PINK).scale(2)
        ...
        self.add(ellipse, F1P, F2P, dot_F1, dot_F2, dot_P)
        self.wait()
