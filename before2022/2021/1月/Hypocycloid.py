    ...
    def construct(self):
        ...
        for j in range(len(r_list)):
            ...
            for i in range(len(d_list[j])):
                curve_i = Hypocycloid(R=R, r=r_list[j], d=d_list[j][i], theta_max=cycle_list[j] * TAU, stroke_width=2.,
                                      color=color_list[i])
                curve_j.add(curve_i)
        ...
        for i in range(len(r_list)):
            text_i = Text(str_list[i][::-1], font='Comic Sans MS', color=GREEN).scale(0.9).to_corner(
                RIGHT * 2.4 + UP * 1.8)
...
class Hypocycloid_transfrom_03(Scene):
    ...
    def construct(self):
        ...
        for j in range(len(r_list)):
            ...
            for i in range(len(d_list[j])):
                curve_i = Hypocycloid(R=R, r=r_list[j], d=d_list[j][i], theta_max=cycle_list[j] * TAU, stroke_width=1.8,
                                      color=color_list[i])
                curve_j.add(curve_i)
...
class Quote_intro(Scene):
    ...
    def construct(self):
        text = TexMobject('Knowledge', '\\ is a pleasure,', 'while', '\\ curiosity', '\\ is the germination of',
