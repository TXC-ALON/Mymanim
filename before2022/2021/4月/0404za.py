...
class mobject(Scene):
    CONFIG = {
        "camera_config": {
            "background_color": BLUE_C
        }
    }

    def construct(self):
        Lb = Logo().set_height(2)
        text = VGroup(
            Text("Manim-Kindergarten", font="Orbitron", color=DARK_GRAY),
            Text("鹤翔万里", font="庞门正道标题体", color=BLACK, size=2.3)
        ).scale(1.2).arrange(DOWN, aligned_edge=LEFT, buff=0.1).next_to(Lb, buff=0.5)
        self.add(Lb, text)
        VGroup(*self.mobjects).set_fill(color=BLACK, opacity=0.5).center()