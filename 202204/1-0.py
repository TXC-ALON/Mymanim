...
class Scene_0(ThreeDScene):
    ...
    def construct(self):
        ...
        frame.set_euler_angles(units=RADIANS)
        captions = [
        ...
        arrow2 = DoubleArrow(np.array([-FRAME_X_RADIUS, 1.5, 1]), np.array([FRAME_X_RADIUS, 1.5, 1]), color=GRAY,
                             buff=0)
        frame_width = VGroup(
            CodeLine("FRAME_WIDTH=FRAME_HEIGHT*", size=0.4),
            CodeLine("PIXEL_WIDTH", size=0.4),
            CodeLine("PIXEL_HEIGHT", size=0.4),
            Line(ORIGIN, RIGHT, color=GREY),
        ...
        self.play(Transform(captions_mob[0], captions_mob[5]),
                  FadeOut(VGroup(arrow1, arrow2, frame_height, frame_width)))
        self.play(ShowCreation(plane))
        self.play(ShowCreation(axes))
        self.play(Write(NumberPlane(y_vals=[1, 2, 3, -1, -2], number_config={"color": BLACK})))
        origin = VGroup(
            Dot(color=ORANGE),
        ...
        large = VGroup(
            ...
            VGroup(
                ...
                CodeLine("LEFT_SIDE", size=0.5).add_background_rectangle(color=WHITE, buff=0.3).next_to(LEFT_SIDE,
                                                                                                        RIGHT)
            ),
            VGroup(
                Vector(RIGHT_SIDE, color=ORANGE),
                CodeLine("RIGHT_SIDE", size=0.5).add_background_rectangle(color=WHITE, buff=0.3).next_to(RIGHT_SIDE,
                                                                                                         LEFT)
            ),
        )
